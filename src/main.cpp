...
int main() {
    ...
    int max_steps = 1500; // Max number of steps to try for before lowering the failure distance
    double failure_cte = 7.0;
    double score = 0.0;

    h.onMessage([&pid, &twiddler, &step, &max_steps, &failure_cte, &score]
...
//                    std::cout << msg << std::endl;
    ...
    h.onConnection([&h, &init](uWS::WebSocket<uWS::SERVER> ws, uWS::HttpRequest req) {
